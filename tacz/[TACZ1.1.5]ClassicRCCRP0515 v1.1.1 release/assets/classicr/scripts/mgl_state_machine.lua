...
function reload_state.entry(this, context)
    ...
    state.need_ammo = context:getMaxAmmoCount() - context:getAmmoCount() + (isNoAmmo and 1 or 0)
    state.loaded_ammo = 0
end
...
function reload_state.transition(this, context, input)
    if (input == this.INPUT_RELOAD_RETREAT or input == INPUT_CANCEL_RELOAD) then
        context:runAnimation("reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
