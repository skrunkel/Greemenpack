...
local AIM_TRACK = increment(blending_track_top)
...
function aim_state.entry(this, context)
    local track = context:getTrack(BLENDING_TRACK_LINE, AIM_TRACK)
    context:runAnimation("ads", track, true, PLAY_ONCE_STOP, 0)
    context:pauseAnimation(track)
end

function aim_state.update(this, context)
    local track = context:getTrack(BLENDING_TRACK_LINE, AIM_TRACK)
    context:resumeAnimation(track)
    context:setAnimationProgress(track,context:getAimingProgress()*0.8 ,true)
    context:pauseAnimation(track)
end
